...
describe('(GET /memes/:id)', () => {
  test('should return a response with status 200 and type JSON', async () => {
    const memeId = 26; // ID del meme, habría que asegurarse de que el id existe en la BBDD
    const response = await request(app).get(`/memes/${memeId}`);
    expect(response.statusCode).toBe(200);
    expect(response.body.id).toBe(memeId); // Verifica que el ID devuelto es correcto
    expect(response.headers['content-type']).toContain('application/json');
  });
...
describe('(DELETE /memes/:id)', () => {
  test('should delete a meme and return it with status 200', async () => {
    const memeId = 35; // ID del meme, habría que asegurarse de que el id existe en la BBDD
    const response = await request(app).delete(`/memes/${memeId}`);
